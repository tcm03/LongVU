...
def eval(args):
    ...
    for i, row in df.iterrows():
        ...
        # qs = "The video is from Youtube and is meant to be published on this platform for users to later watch and interact. The engagement rate is calculated based on the number of likes and dislikes the video will receive when published onto Youtube. The formula for the engagement rate is (likes-dislikes) / (likes+dislikes) where the score is between -1 and 1, inclusive. The final label is either 0 (not engaged), 1 (neutral), or 2 (engaged) based on thresholding the engagement rate. Please predict the label for this video first by printing out either 0, 1, or 2 (you have to print one of these 3 numbers in your response). Then, explain in words why the video is not engaged, neutral, or engaged based on its content. For example: Engagement rate label prediction: 0. The video is not engaging because it is boring and lacks interesting content. Another example: Engagement rate label prediction: 2. The video is engaging because it is informative and contains educational value."
        qs = "Please describe this video."
